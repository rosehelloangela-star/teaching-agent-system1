...
class TeachingKnowledgeGraph:
    ...
    def get_case_groups(self, concept: str) -> Dict[str, List[str]]:
        ...
        with self.driver.session() as session:
            for record in session.run(query, c=concept):
                ...
                if relation == "HAS_POSITIVE_CASE" or "PositiveCase" in labels:
                    bucket = "positive"
                elif relation == "HAS_NEGATIVE_CASE" or "NegativeCase" in labels:
                    bucket = "negative"
                else:
                    bucket = "general"
        ...
        return {key: self._dedupe_texts(value) for key, value in grouped.items()}

    def get_positive_cases(self, concept: str) -> List[str]:
        return self.get_case_groups(concept).get("positive", [])

    def get_negative_cases(self, concept: str) -> List[str]:
        return self.get_case_groups(concept).get("negative", [])
    ...
    def get_learning_subgraph(self, detected_concepts: List[str], max_triples: int = 30) -> Dict[str, Any]:
        ...
        for concept in all_target_concepts:
            ...
            for item in case_groups.get("positive", []):
                positive_cases[concept].append(item)
        ...
        has_real_case = any(
            len(items) > 0
            for items in list(case_examples.values()) + list(positive_cases.values()) + list(negative_cases.values())
        )

        return {
            "hit_nodes": detected_concepts,
            "expanded_nodes": expanded_concepts,
            "triples": triples,
            "case_examples": {key: self._dedupe_texts(value) for key, value in case_examples.items()},
            "positive_cases": {key: self._dedupe_texts(value) for key, value in positive_cases.items()},
            "negative_cases": {key: self._dedupe_texts(value) for key, value in negative_cases.items()},
            "mistakes": {key: self._dedupe_texts(value) for key, value in mistakes.items()},
            "missing_prereqs": self._dedupe_texts(missing_prereqs),
            "fallback_case_needed": not has_real_case,
        }
