...
from app.agents.competition_templates import (
    TEMPLATE_REGISTRY,
    build_dimension_question_blueprint,
    build_template_judge_panel,
)
from app.agents.mechanism.llm_config import get_llm
from app.agents.roles import ROLE_CONFIG_REGISTRY
from app.core.json_utils import extract_first_json_value, message_content_to_text, parse_pydantic_from_text
...
class _CompetitionJudgeQuestion(BaseModel):
    # 这里对 structurer 的原始输出做宽松校验：
    # 大模型可能漏掉部分字段，后续会在 _sanitize_judge_questions 中
    # 基于模板蓝图与维度信息自动补齐，避免因为单个字段缺失而整轮失败。
    expert_id: str = ""
    expert_role: str = ""
    expert_domain: str = ""
...
class _CompetitionStructOutput(BaseModel):
    ...
    top_tasks: List[_CompetitionTask] = Field(default_factory=list, max_length=3)
    quick_questions: List[str] = Field(default_factory=list, max_length=3)
    judge_questions: List[_CompetitionJudgeQuestion] = Field(default_factory=list, max_length=3)
...
def _pick_competition_ranked_dimensions(items: List[Dict[str, Any]], limit: int = 3) -> List[Dict[str, Any]]:
    ranked = sorted(items, key=lambda x: (x.get("estimated_score", 5), -x.get("weight", 0), x.get("dimension_name", "")))
    return ranked[:limit]


def _format_question_brief(card: Dict[str, Any]) -> str:
    role = card.get("expert_role") or "评委"
    question = (card.get("question") or "").strip()
    return f"【{role}】{question}" if question else f"【{role}】请补齐该维度的核心证据。"


def _render_judge_question_markdown(card: Dict[str, Any]) -> str:
    role = card.get("expert_role") or "评委"
    dimension_name = card.get("target_dimension_name") or "关键维度"
    pressure = card.get("pressure_level") or "高压追问"
    question = (card.get("question") or "请补齐该维度的关键证据。")
    attack = (card.get("attack_point") or "该维度的核心论证还不够扎实。")
    evidence_hint = (card.get("evidence_hint") or "请准备与该维度直接对应的客观材料。")
    return (
        f"- **{role}｜{dimension_name}｜{pressure}**\n"
        f"  > {question.strip()}\n"
        f"  - 盯住薄弱点：{attack.strip()}\n"
        f"  - 回答时先补：{evidence_hint.strip()}"
    )


def _build_rule_based_judge_questions(items: List[Dict[str, Any]], template_ctx: Dict[str, Any], limit: int = 3) -> List[Dict[str, Any]]:
    template_id = template_ctx.get("template_id") or ""
    ranked_items = _pick_competition_ranked_dimensions(items, limit=max(limit + 2, limit))
    ...
    for index, item in enumerate(ranked_items):
        ...
        if not dimension_id or dimension_id in used_dimensions:
            continue
        blueprint = build_dimension_question_blueprint(template_id, dimension_id)
        templates = blueprint.get("question_templates") or []
        question_template = templates[index % len(templates)] if templates else "请你直接说明 {dimension_name} 的核心证据在哪里。"
        evidence_bits = item.get("missing_evidence", [])[:2]
        evidence_hint = "；".join(evidence_bits) or blueprint.get("evidence_hint") or "请补齐该维度的关键证据"
        question = question_template.format(
            dimension_name=item.get("dimension_name") or dimension_id,
            evidence_hint=evidence_hint,
            attack_point=blueprint.get("attack_point") or "",
            weight=item.get("weight"),
            score=item.get("estimated_score"),
        )
        expert_id = blueprint.get("expert_id") or ""
        if expert_id in used_experts and len(ranked_items) > len(questions) + 1:
            continue
        questions.append({
            "expert_id": expert_id,
            "expert_role": blueprint.get("expert_role") or "评委",
            "expert_domain": blueprint.get("expert_domain") or "综合",
            "target_dimension_id": dimension_id,
            "target_dimension_name": item.get("dimension_name") or dimension_id,
            "pressure_level": blueprint.get("pressure_level") or "高压追问",
            "attack_point": blueprint.get("attack_point") or "质疑该维度的核心论证是否成立",
    ...
    if not questions:
        fallback_panel = build_template_judge_panel(template_id)
        fallback_expert = fallback_panel[0] if fallback_panel else {
            "expert_id": "general_judge",
            "expert_role": "综合评委",
            "expert_domain": "综合",
            "style_tag": "追问",
        }
        questions.append({
            "expert_id": fallback_expert.get("expert_id", "general_judge"),
            "expert_role": fallback_expert.get("expert_role", "综合评委"),
            "expert_domain": fallback_expert.get("expert_domain", "综合"),
            "target_dimension_id": "general",
            "target_dimension_name": "整体论证",
            "pressure_level": "高压追问",
            "attack_point": "质疑整体论证是否有足够证据支撑",
            "question": "如果评委现在要求你只用最硬的一页材料说服他，你会展示哪一页，为什么不是别页？",
            "evidence_hint": "准备最能证明核心主张的关键材料。",
        })
    return questions[:limit]


def _sanitize_judge_questions(raw_questions: List[Dict[str, Any]], items: List[Dict[str, Any]], template_ctx: Dict[str, Any], limit: int = 3) -> List[Dict[str, Any]]:
    item_map = {item.get("dimension_id"): item for item in items}
    ...
    for raw in raw_questions or []:
        if not isinstance(raw, dict):
            continue
        raw = raw or {}
        dimension_id = raw.get("target_dimension_id") or raw.get("dimension_id") or ""
        dimension_item = item_map.get(dimension_id)
        if not dimension_item:
            dimension_name = str(raw.get("target_dimension_name") or "")
            dimension_item = next((item for item in items if item.get("dimension_name") == dimension_name), None)
            if dimension_item:
                dimension_id = dimension_item.get("dimension_id")
        if not dimension_item or not dimension_id or dimension_id in used_dimensions:
            continue
        blueprint = build_dimension_question_blueprint(template_ctx.get("template_id") or "", dimension_id)
        evidence_bits = dimension_item.get("missing_evidence", [])[:2]
        evidence_hint = str(raw.get("evidence_hint") or "；".join(evidence_bits) or blueprint.get("evidence_hint") or "请补齐该维度的关键证据").strip()
        question = str(raw.get("question") or "").strip()
        if not question:
            fallback = _build_rule_based_judge_questions([dimension_item], template_ctx, limit=1)[0]
            question = fallback.get("question", "请补齐该维度的关键证据。")
        sanitized.append({
            "expert_id": str(raw.get("expert_id") or blueprint.get("expert_id") or "").strip(),
            "expert_role": str(raw.get("expert_role") or blueprint.get("expert_role") or "评委").strip(),
            "expert_domain": str(raw.get("expert_domain") or blueprint.get("expert_domain") or "综合").strip(),
            "target_dimension_id": dimension_id,
            "target_dimension_name": str(raw.get("target_dimension_name") or dimension_item.get("dimension_name") or dimension_id).strip(),
            "pressure_level": str(raw.get("pressure_level") or blueprint.get("pressure_level") or "高压追问").strip(),
            "attack_point": str(raw.get("attack_point") or blueprint.get("attack_point") or "质疑该维度的核心论证是否成立").strip(),
            "question": question,
            "evidence_hint": evidence_hint,
        })
        used_dimensions.add(dimension_id)
    ...
    if len(sanitized) < limit:
        existing = {item.get("target_dimension_id") for item in sanitized}
        remaining_items = [item for item in _pick_competition_ranked_dimensions(items, limit=limit + 3) if item.get("dimension_id") not in existing]
        sanitized.extend(_build_rule_based_judge_questions(remaining_items, template_ctx, limit=limit - len(sanitized)))
...
def _enrich_competition_content(parsed_dict: Dict[str, Any], template_ctx: Dict[str, Any]) -> Dict[str, Any]:
        ...
        merged_items.append(
            {
                ...
                "risk_level": _score_to_risk(score),
                "low_score_guardrail_pass": True,
                "primary_expert_role": template_item.get("primary_expert_role"),
                "primary_expert_domain": template_item.get("primary_expert_domain"),
                "question_focus": template_item.get("question_focus"),
            }
        )
    total_weight = sum(item["weight"] for item in merged_items) or 100
    weighted_score_pct = round(sum(item["estimated_score"] / 5 * item["weight"] for item in merged_items), 1)
    ...
    judge_questions = _sanitize_judge_questions(parsed_dict.get("judge_questions", []) or [], merged_items, template_ctx, limit=3)
    quick_questions = [_format_question_brief(card) for card in judge_questions]
    ...
    competition_meta = {
        ...
        "total_weight": total_weight,
        "exclusive_dimensions": template_ctx.get("exclusive_dimensions", []),
        "judge_panel": template_ctx.get("judge_panel", build_template_judge_panel(template_ctx.get("template_id") or "")),
    }
    ...
    enriched["score_summary"] = score_summary
    enriched["panel_charts"] = panel_charts
    enriched["judge_questions"] = judge_questions
    enriched["quick_questions"] = quick_questions
    enriched["rubric_scores"] = _build_competition_rubric_markdown(merged_items)
    enriched["deduction_evidence"] = _build_competition_deduction_text(merged_items)
    return enriched
...
def _build_competition_template_digest(template_ctx: Dict[str, Any]) -> str:
    ...
    digest = {
        ...
        "focus_hint": template_ctx.get("focus_hint"),
        "expected_shift": template_ctx.get("expected_shift"),
        "judge_panel": template_ctx.get("judge_panel", []),
        "exclusive_dimensions": template_ctx.get("exclusive_dimensions", []),
        "rubric_items": [
            {
                "dimension_id": item.get("dimension_id"),
                ...
                "primary_expert_domain": item.get("primary_expert_domain"),
...
def _build_rule_based_competition_reply(content: Dict[str, Any]) -> str:
    ...
    items = content.get("rubric_items", [])
    top_tasks = content.get("top_tasks", [])
    judge_questions = content.get("judge_questions", [])[:3]
    ...
    strong_text = "、".join(item.get("dimension_name", "") for item in strongest) or "暂未形成明显强项"
    question_lines = [_render_judge_question_markdown(card) for card in judge_questions] or [
        _render_judge_question_markdown({
            "expert_role": "综合评委",
            "target_dimension_name": "整体论证",
            "pressure_level": "高压追问",
            "question": "如果评委追问“你的核心证据到底在哪一页”，你准备展示哪一页材料？",
            "attack_point": "质疑整体论证是否有足够证据支撑",
            "evidence_hint": "准备最能证明核心主张的关键材料。",
        })
    ]
...
def _run_competition_structurer(state: dict, schema, output_contract: str, competition_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    struct_contract = (
        ...
        '      "expert_domain": "商业/财务",\n'
        '      "target_dimension_id": "business_finance",\n'
        '      "target_dimension_name": "商业模式与财务回报",\n'
        '      "pressure_level": "盈利追问",\n'
        '      "attack_point": "质疑付费闭环与单位经济是否成立",\n'
        '      "question": "到底谁付钱、为什么现在付、你凭什么不是越卖越亏？",\n'
        '      "evidence_hint": "收入模式、成本结构、定价依据"\n'
        '    }\n'
        '  ],\n'
        '  "quick_questions": ["【商业模式与财务评委】到底谁付钱、为什么现在付、你凭什么不是越卖越亏？"]\n'
        '}'
    )
    ...
    system_text = (
        ...
        "4. missing_evidence 必须指向真实缺口，例如缺访谈、缺竞品矩阵、缺单位经济、缺论文/专利支撑。\n"
        "5. top_tasks 最多 3 条，优先高权重且低分维度。\n"
        "6. judge_questions 最多 3 条，每条都必须明确写出：是哪一类评委、主要针对哪个 Rubric 维度、他正在质疑的薄弱点是什么、问题本身怎么问。\n"
        "7. judge_questions 必须像冷酷但专业的比赛评委追问，不能写成泛泛建议，也不能只给一句安慰式提醒。\n"
        "8. quick_questions 允许为空；若填写，应与 judge_questions 一一对应并写成简短兼容字段。\n"
        "模板摘要：{template_digest}\n"
        "输出格式参考：{struct_contract}"
    )
    user_text = (
    ...
    prompt = ChatPromptTemplate.from_messages([("system", system_text), ("user", user_text)])
    chain = prompt | get_llm(temperature=0.1, max_tokens=1800)
    ...
    if "top_tasks" not in struct_content or not struct_content.get("top_tasks"):
        struct_content["top_tasks"] = (json.loads(tasks_text) if tasks_text else [])[:3]
    if "judge_questions" not in struct_content:
        struct_content["judge_questions"] = []
    if "quick_questions" not in struct_content:
        struct_content["quick_questions"] = []
...
def _run_competition_presenter(state: dict, enriched_content: Dict[str, Any]) -> str:
    ...
    presenter_payload = {
        ...
        "weakest_dimensions": weakest,
        "top_tasks": top_tasks,
        "judge_questions": enriched_content.get("judge_questions", [])[:3],
        "rubric_items": [
            {
                "dimension_name": item.get("dimension_name"),
                "weight": item.get("weight"),
    ...
    system_text = (
        ...
        "3. 必须优先谈高权重短板，而不是平均用力。\n"
        "4. 必须结合具体证据缺口，不要写空话。\n"
        "5. 在“### ❓ 路演追问预演”中，必须逐条写成“**XX领域评委｜针对XX维度｜XX追问**”的形式，再给出问题正文与盯住的薄弱点。\n"
        "6. 不要重新发明评分，只能基于已提供的结构化 judge_questions 与 rubric 结果进行表达。\n"
        "输出格式：{{\"reply\": \"Markdown 文本\"}}"
    )
    user_text = "请根据以下已经确定的评分结果写 reply：\n{presenter_payload}"
    prompt = ChatPromptTemplate.from_messages([("system", system_text), ("user", user_text)])
    chain = prompt | get_llm(temperature=0.35, max_tokens=1600)
    ...
    if role_id == "competition_advisor":
        ...
        try:
            ...
            final_content = {
                ...
                "rubric_items": enriched_content.get("rubric_items", []),
                "top_tasks": struct_content.get("top_tasks", [])[:3],
                "judge_questions": struct_content.get("judge_questions", [])[:3],
                "quick_questions": struct_content.get("quick_questions", [])[:3],
            }
            final_content = _enrich_competition_content(final_content, competition_context)
            schema.model_validate(final_content)
