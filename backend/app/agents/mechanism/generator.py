...
#     parsed_ids = [item.get("dimension_id") for item in parsed_items]

#     if set(parsed_ids) != set(expected_ids) or len(parsed_ids) != len(expected_ids):
#         raise ValueError(
#             "竞赛模式输出未完整覆盖模板维度。"
#             f"期望={expected_ids}，实际={parsed_ids}"
#         )

#     duplicate_ids = {item_id for item_id in parsed_ids if parsed_ids.count(item_id) > 1}
#     if duplicate_ids:
#         raise ValueError(f"竞赛模式输出存在重复维度：{sorted(duplicate_ids)}")

#     for item in parsed_items:
#         score = float(item.get("estimated_score", 0))
#         missing_evidence = item.get("missing_evidence") or []
#         fix_24 = str(item.get("minimal_fix_24h", "")).strip()
#         fix_72 = str(item.get("minimal_fix_72h", "")).strip()
#         if score <= 2 and len(missing_evidence) < 1:
#             raise ValueError(f"维度 {item.get('dimension_id')} 得分<=2，但未提供 Missing Evidence。")
#         if score <= 2 and (not fix_24 or not fix_72):
#             raise ValueError(f"维度 {item.get('dimension_id')} 得分<=2，但 24h/72h 修复建议不完整。")
...
#         item = parsed_items[template_item["dimension_id"]]
#         score = round(float(item.get("estimated_score", 0)), 1)
#         merged_items.append(
#             {
...
#                 "low_score_guardrail_pass": not (
#                     score <= 2 and (
#                         len(item.get("missing_evidence", []) or []) < 1
#                         or not str(item.get("minimal_fix_24h", "")).strip()
#                         or not str(item.get("minimal_fix_72h", "")).strip()
#                     )
#                 ),
#             }
#         )
...
#     template_json = json.dumps(competition_context, ensure_ascii=False, indent=2) if competition_context else "{}"

#     llm = get_llm(temperature=0.25, max_tokens=3200 if role_id == "competition_advisor" else 2200)
#     output_contract = _build_output_contract(role_id)
...
#             "   - 使用 ### 作为小标题来区分段落。\n"
#             "2. 反代写红线：如果学生要求“直接写”、“生成商业计划书”、“帮我完善这段话”等代写请求，必须设 is_refused = true。\n"
#             "3. 显性结构化：reply 中必须依次包含概念解析、项目案例、避坑指南、实操任务、交付要求、评价标准。\n"
#             "4. 结尾必须抛出一个启发性问题。\n"
#         )
#     elif role_id == "project_coach":
#         system_prompt_text += (
#             "\n【双轨任务指令（最高优先级）】\n"
#             "你现在肩负双重任务：1. 生成给前端面板的后台结构化数据；2. 在 reply 字段中生成与学生的真实对话。\n"
#             "reply 必须使用 Markdown，且按顺序包含：项目所处阶段、当前核心诊断、诊断证据追踪、风险预警、破局任务。\n"
#             "最后必须追加 1~2 个具有压力测试性质的苏格拉底式追问。\n"
#         )
#     elif role_id == "competition_advisor":
#         system_prompt_text += (
#             "\n【竞赛模式双轨输出规则（最高优先级）】\n"
#             "你现在要同时完成两件事：\n"
#             "1. 在 reply 中生成给学生看的 Markdown 富文本评审反馈；\n"
#             "2. 在 rubric_items / top_tasks / quick_questions 中生成给系统面板用的结构化数据。\n"
#             "\n【赛事模板约束】\n"
#             "- 你必须严格使用系统提供的 rubric 维度，不能新增、不能缺漏、不能改名。\n"
#             "- rubric_items 必须 100% 覆盖模板维度，而且每个 dimension_id 只能出现一次。\n"
#             "- 模板中的 weight 已经物理切换，你必须在 reply 中明确体现权重偏移与高权重维度。\n"
#             "- 当 estimated_score <= 2 时，missing_evidence 至少 1 条，且必须同时给出 minimal_fix_24h 与 minimal_fix_72h。\n"
#             "\n【reply 的排版要求】\n"
#             "reply 必须使用 Markdown，且按顺序至少包含以下 6 个模块：\n"
#             "### 🏁 赛事口径识别\n"
#             "### 📊 总体预判\n"
#             "### 🎯 高权重扣分项\n"
#             "### 🧩 逐项修复思路\n"
...
#             "### ❓ 路演追问预演\n"
#             "\n【结构化输出要求】\n"
#             "- rubric_items 中的 score_reason 必须针对当前材料说人话，不能只写空泛句。\n"
#             "- top_tasks 最多3条，优先挑高权重、低分且短期可修复的维度。\n"
#             "- quick_questions 最多3条，必须像冷酷评委会在现场追问的问题。\n"
#             "\n【当前赛事模板 JSON】\n"
#             "{template_json}\n"
#         )
...
#                 "diagnosis": diagnosis_text,
#                 "tasks": tasks_text,
#                 "document_excerpt": _truncate_text(bound_document_text, max_chars=5000) if role_id == "competition_advisor" else "无",
#                 "errors": errors,
#             }
#         )
#         raw_text = message_content_to_text(response).strip()
...
#         if not raw_text:
#             raise ValueError("大模型返回了完全空白的字符串。")
...
def _build_competition_prompt(compact: bool = False):
    if not compact:
        system_text = (
            ...
            "\n【竞赛模式硬约束】\n"
            "1. 必须严格覆盖模板中的全部 rubric_items，不能新增、缺漏、改名。\n"
            "2. dimension_id 必须逐项对应模板。\n"
            "3. 当 estimated_score <= 2 时，missing_evidence 至少1条，且 24h/72h 修复都必须有。\n"
            "4. top_tasks 最多3条，优先高权重且低分维度。\n"
            "5. quick_questions 最多3条，必须像评委现场追问。\n"
            "6. 所有文字尽量精炼，但 reply 必须有足够信息密度，不能过于敷衍。\n"
            ...
            "### ❓ 路演追问预演\n"
            "\n【reply 写作要求】\n"
            "- 必须明确指出当前识别到的赛事名称与其偏好的评价重点。\n"
            "- 必须结合项目真实漏洞，给出至少 3 个高权重短板。\n"
            "- 必须至少点出 2 条最关键的缺失证据。\n"
            "- 必须用评委口吻，既专业又带压力测试感。\n"
            "- 必须避免空洞鼓励话术。\n"
            "\n【赛事模板摘要】\n{template_digest}\n"
        ...
        system_text = (
            ...
            "- estimated_score <=2 时，必须给 missing_evidence、minimal_fix_24h、minimal_fix_72h。\n"
            "- reply 只写简洁 Markdown，总结赛事口径、总分预判、高权重短板、24h/72h建议。\n"
            "- 严禁输出多余文字。\n"
...
def generator_node(state: dict) -> dict:
    ...
    if role_id == "competition_advisor":
        ...
        normal_chain = normal_prompt | get_llm(temperature=0.18, max_tokens=1900)
        normal_payload = {
            "output_contract": output_contract,
        ...
        try:
            ...
            log_and_emit(state, "generator", "首轮生成超时，自动切换精简竞赛提示词重试一次。", level="warning")
            compact_prompt = _build_competition_prompt(compact=True)
            compact_chain = compact_prompt | get_llm(temperature=0.1, max_tokens=1200)
            compact_payload = {
                "output_contract": output_contract,
                "template_digest": template_digest,
                "user_input": user_input[:300],
                "diagnosis": _truncate_text(diagnosis_text, max_chars=500),
                "tasks": _truncate_text(tasks_text, max_chars=400),
                "document_excerpt": _truncate_text(bound_document_text, max_chars=700),
            }
            try:
                ...
                log_and_emit(state, "generator", f"模型调用或解析失败：{e2}", level="error")
                raise ValueError(f"生成器执行失败，未能生成有效格式：{str(e2)}")

    # ====================
    # 以下学习模式 / 项目模式逻辑严格恢复为原版，不做功能性改写
    # ====================
    llm = get_llm(temperature=0.3, max_tokens=2000)
