...
def critic_node(state: dict) -> dict:
    ...
    if has_bound_doc:
        ...
        # 在 critic.py 中找到 current_mode in {"project", "competition"} 这一段
    ...
    engine_context = ""
    hypergraph_state_data = {} # 【新增】用来存储超图数据并返回给前端
    ...
    if current_mode == "learning":
        ...
        try:
            ...
            # fuzzy_hg_nodes 仅作为底层暗桩提醒大模型，绝不传给前端画图
            fuzzy_hg_nodes = detect_hyper_nodes(analysis_source_text, fuzzy_threshold=72, debug=True)
            
            # 【核心修复 1】：前端图表 nodes 数组置空，只依靠大模型提取的具体 edges 来自动生成画板，保证画布纯净
            hypergraph_state_data = {
                "nodes": [], 
                "edges": extracted_h_edges or {}
            }
            
            # 【核心修复 2】：去掉 if extracted_h_edges 判断，不管提没提取到，强制启动引擎进行审判！
            ...
            if alerts or fuzzy_hg_nodes:
                ...
                for alert in alerts:
                    # 【核心修复】：全部换成 .get() 安全获取，永远不会再报 KeyError
    ...
    try:
        ...
        return {
            ...
            "hypergraph_data": hypergraph_state_data, # 【修改】将超图数据返回
            "llm_unavailable": False,
        }
    except Exception as e:
        elapsed = time.perf_counter() - start
        ...
        return {
            ...
            "hypergraph_data": hypergraph_state_data, # 【修改】失败时也传递
