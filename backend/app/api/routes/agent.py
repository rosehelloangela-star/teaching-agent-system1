...
# from app.agents.graph_workflow import app as agent_workflow
# from app.agents.graph_workflow import build_initial_state
# from app.schemas.agent import AgentRunResponse
# from app.core.file_utils import extract_text_from_file
# from app.core.stream_logger import (
#     emit_done,
#     emit_error,
#     emit_final,
#     emit_log,
...
# def _build_agent_response(final_state: dict, current_mode: str, thread_id: str) -> dict:
#     return {
...
#         for file in files:
#             if file.filename:
#                 file_texts += await extract_text_from_file(file)
...
#         return AgentRunResponse(**_build_agent_response(final_state, current_mode, thread_id))
...
#             emit_final(thread_id, _build_agent_response(final_state, current_mode, thread_id))
#         except Exception as e:
...
def _payload_to_prompt_text(payload: dict) -> str:
    if not payload.get("supported"):
        return f"\n\n--- 附件：{payload.get('filename', '未命名文件')} （{payload.get('error', '解析失败')}） ---\n"

    return (
        f"\n\n--- 附件：{payload.get('filename', '未命名文件')} 开始 ---\n"
        f"{payload.get('text', '')}\n"
        f"--- 附件：{payload.get('filename', '未命名文件')} 结束 ---\n"
    )


def _merge_analysis_snapshot(conversation: Conversation, final_payload: dict, current_mode: str) -> dict:
    ...
    snapshot["document"] = {
        ...
        "bound_file_uploaded_at": conversation.bound_file_uploaded_at.isoformat()
        if conversation.bound_file_uploaded_at
        else None,
    }

    snapshot["last_mode"] = current_mode
    snapshot["updated_at"] = datetime.utcnow().isoformat()

    if current_mode in {"learning", "project", "competition"}:
        snapshot[current_mode] = {
            "updated_at": datetime.utcnow().isoformat(),
            "critic_diagnosis": final_payload.get("critic_diagnosis", {}),
            "planned_tasks": final_payload.get("planned_tasks", []),
            "generated_content": final_payload.get("generated_content", {}),
        }
...
async def run_agent(
    ...
    try:
        ...
        # 兼容旧逻辑：非 conversation 的请求，仍按“本轮附件拼接”处理
        file_texts = ""
