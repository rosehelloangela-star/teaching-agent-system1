...
//   System: ['Core_Business_Loop', 'Public_Welfare_Targeting'],
...
//   PR3: ['Public_Welfare_Value_Design', 'Public_Welfare_Demand_Evidence'],
...
//   PR12: ['Volunteer_Operations', 'Stakeholder_Collaboration'],
//   PR13: ['Resource_Sustainability', 'Fundraising_Model'],
//   PR14: ['Policy_Compliance_Public', 'Public_Welfare_Targeting', 'Public_Welfare_Value_Design'],
...
//   public_welfare_targeting: '公益对象界定',
//   public_welfare_demand_evidence: '公益需求证据',
//   public_welfare_value_design: '公益服务设计',
//   stakeholder_collaboration: '关键协同网络',
//   public_welfare_ethics_safeguard: '公益伦理保护',
//   fundraising_model: '公益筹资路径',
//   benefit_cost_efficiency: '公益成本效率',
//   impact_measurement: '影响评估可验证性',
//   beneficiary_conversion_path: '受益者转化路径',
//   public_trust_transparency: '公信力与透明度',
//   pilot_replication: '试点复制扩散',
//   volunteer_operations: '志愿者运营治理',
//   resource_sustainability: '资源续航能力',
//   policy_compliance_public: '公益政策合规',
//   social_value_cocreation: '社会共创机制',
// };
...
//   const projectTypeLabel = projectStageFlow?.project_type_label || '商业项目';
...
//       'R&D_Team_Match': '研发团队匹配', Resource_Feasibility: '资源方案可行性',
...
//       Stakeholder_Collaboration: '关键协同网络',
//       Public_Welfare_Ethics_Safeguard: '公益伦理保护',
//       Fundraising_Model: '公益筹资模型',
//       Benefit_Cost_Efficiency: '公益成本效率',
//       Impact_Measurement: '影响评估体系',
//       Beneficiary_Conversion_Path: '受益者转化路径',
//       Public_Trust_Transparency: '公信力与透明度',
//       Pilot_Replication: '试点复制扩散',
//       Volunteer_Operations: '志愿者运营',
//       Resource_Sustainability: '资源可持续性',
//       Policy_Compliance_Public: '公益政策合规',
//       Social_Value_CoCreation: '社会共创机制',
//       Beneficiary_Group: '受益对象', Urgency_Pain: '紧迫痛点', Service_Scenario: '服务场景', Accessibility_Constraint: '可达性约束',
//       Research_Sample: '调研样本', Field_Observation: '实地观察', Needs_Quote: '需求引述', Problem_Severity: '问题严重度',
//       Intervention_Solution: '干预方案', Expected_Outcome: '预期成效', Core_Service: '核心服务', Trust_Mechanism: '信任机制',
//       Government_Link: '政府链接', NGO_Partner: '公益伙伴', Community_Leader: '社区关键人', Volunteer_Source: '志愿者来源',
//       Vulnerable_Group: '弱势群体', Ethical_Risk: '伦理风险', Privacy_Protection: '隐私保护', Consent_Process: '知情同意流程',
//       Funding_Source: '资金来源', Donation_Product: '捐赠产品', Grant_Channel: '资助渠道', Revenue_Supplement: '补充造血收入',
//       Single_Service_Cost: '单次服务成本', Management_Cost: '管理成本', Fund_Use_Ratio: '资金使用比例', Budget_Ceiling: '预算上限',
//       Impact_Goal: '影响目标', Indicator_System: '指标体系', Baseline_Data: '基线数据', Evaluation_Method: '评估方法',
//       Reach_Channel: '触达渠道', Participation_Motivation: '参与动机', Retention_Mechanism: '留存机制', Referral_Path: '转介绍路径',
//       Disclosure_Frequency: '披露频率', Financial_Disclosure: '财务披露', Story_Evidence: '故事证据', Third_Party_Endorsement: '第三方背书',
//       Pilot_Site: '试点点位', Pilot_Result: '试点结果', Replication_Condition: '复制条件', Expansion_Path: '扩散路径',
//       Volunteer_Role: '志愿者角色', Training_Process: '培训流程', Scheduling_Mechanism: '排班机制', Incentive_NonCash: '非现金激励',
//       Key_Resource: '关键资源', Resource_Gap: '资源缺口', Replacement_Plan: '替代方案', Risk_Buffer: '风险缓冲',
//       Policy_Basis: '政策依据', Qualification_Requirement: '资质要求', Safety_Redline: '安全红线', Public_Opinion_Risk: '舆情风险',
//       Enterprise_Partner: '企业伙伴', School_Hospital_Community: '学校/医院/社区协同', CoCreation_Mode: '共创模式', Longterm_Mechanism: '长期机制',
//     };
...
${rule.issue}` : ''}`}</title>
  ...
  const competitionItems = competition?.rubric_items || [];
  const topTasks = competition?.top_tasks || [];
  const judgeQuestions = competition?.judge_questions || [];


  const judgeToneClasses = (level = '') => {
    const value = String(level || '');
    if (value.includes('高压') || value.includes('极限')) return 'bg-red-50 border-red-200 text-red-700';
    if (value.includes('证据') || value.includes('逻辑') || value.includes('差异')) return 'bg-amber-50 border-amber-200 text-amber-700';
    ...
    <div className="space-y-4">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
        <div className="flex items-center gap-2 text-sm font-bold text-slate-800 mb-3">
          <AlertTriangle size={16} className="text-amber-600" /> 评委追问预演
        </div>
        <div className="space-y-3">
          {judgeQuestions.length > 0 ? judgeQuestions.slice(0, 3).map((item, idx) => (
            <div key={`${item.question || 'judge'}-${idx}`} className="bg-white rounded-xl border border-slate-200 p-3 shadow-sm">
              ...
              <div className="mt-1 text-xs text-slate-400">回答时优先补：{item.evidence_hint || '对应维度的客观材料。'}</div>
            </div>
          )) : (
            <ul className="list-disc pl-5 space-y-2 text-sm text-slate-600">
              {(competition.quick_questions || []).map((q, idx) => <li key={`${q}-${idx}`}>{q}</li>)}
