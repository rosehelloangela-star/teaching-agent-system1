...
function buildTopologyModel({ hypergraph, projectStageFlow, formatNodeName }) {
...
export default function SnapshotOverlay({ open, snapshot, onClose }) {
  ...
  const renderHypergraph = () => {
    const topologyModel = buildTopologyModel({ hypergraph, projectStageFlow, formatNodeName });
    ...
    const getRulePillStyle = (rule) => {
      if (rule.status === 'resolved') {
        return { fill: '#ecfdf5', stroke: '#34d399', text: '#047857', badge: '已突破' };
      }
      if (rule.status === 'locked') {
        return { fill: '#f8fafc', stroke: '#cbd5e1', text: '#94a3b8', badge: '未解锁' };
      }
      if (rule.status === 'active') {
        if (rule.severity === 'critical') return { fill: '#fff1f2', stroke: '#fb7185', text: '#be123c', badge: '高危' };
        if (rule.severity === 'high') return { fill: '#fff7ed', stroke: '#fb923c', text: '#c2410c', badge: '高风险' };
        return { fill: '#fffbeb', stroke: '#f59e0b', text: '#b45309', badge: '待验证' };
      }
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                {!ruleDetail && !conceptDetail ? (
                  <div className="text-sm text-slate-500 leading-relaxed">默认展示当前阶段的整体拓扑。可点击规则节点或矩阵单元格，查看“某条规则为什么打到某个概念”以及它对应的证据缺口。</div>
                ) : (
